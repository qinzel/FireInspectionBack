					...
					doms.each(function() {
						...
						dom.unbind().click(function() {
							...
							if(!$(".date_click").length) {
									...
									if(dom.hasAttribute("startdom_maxdate"))
		                             {
		                             	...
		                             	var selected_year = $(".date_click .year_month select:first").val();
		                             	end_time=stringTotime(dom.attr("startdom_maxdate"))-set_json.startToend_days*24*60*60*1000;
		                             	var curr_month=1;
		                             	while(curr_month<=12)
		                             	{
		                             		if(new Date(selected_year,curr_month-1,1).valueOf()<=end_time)
		                             		{
		                             			ok_month.push(curr_month)
		                             		}
		                             		curr_month++
		                             	}
		                                ...
		                                start_time= stringTotime(dom.attr("enddom_mindate"))+set_json.startToend_days*24*60*60*1000;
		                             	setYear(new Date(start_time).getFullYear());
		                             	var selected_year = $(".date_click .year_month select:first").val();
		                             	var curr_month=1;
		                             	while(curr_month<=12)
		                             	{
		                             		if(curr_month<12&&new Date(selected_year,curr_month,1).valueOf()>=start_time)
		                             		{
		                             			ok_month.push(curr_month)
		                             		}else if(curr_month==12){
		                             			if(new Date(selected_year-0+1,0,1).valueOf()>=start_time)
		                             			{
		                             				ok_month.push(curr_month)
		                             			}
								...
								setMonth(ok_month[0], ok_month[ok_month.length-1]);
								setDays();
								...
								click_change();
                                 ...
                                 if(dom.hasAttribute("startdom_maxdate"))
                                 {
                                 	setYear(date_obj.getFullYear());
                                 	var selected_year = $(".date_click .year_month select:first").val();
                                 	var end_time=stringTotime(dom.attr("startdom_maxdate"))-set_json.startToend_days*24*60*60*1000;
                                 	var curr_month=1;
                                 	while(curr_month<=12)
	                             	{
	                             		if(curr_month<12&&new Date(selected_year,curr_month,1).valueOf()>=todayTime())
	                             		{
	                             			if(new Date(selected_year,curr_month-1,1).valueOf()<=end_time)
	                             			{
	                             			   ok_month.push(curr_month);
	                             			...
	                             			if(new Date(selected_year-0+1,0,1).valueOf()>=todayTime())
	                             			{
	                             				if(new Date(selected_year,curr_month-1,1).valueOf()<=end_time)
		                             			{
		                             			   ok_month.push(curr_month);
                                 	...
                                 	var end_time=todayTime()+(set_json.limit_days>0?set_json.limit_days:0)*24*60*60*1000;
                                 	var start_time=stringTotime(dom.attr("enddom_mindate"))+set_json.startToend_days*24*60*60*1000;
                                 	setYear(new Date(start_time).getFullYear());
                                 	var selected_year = $(".date_click .year_month select:first").val();
                                 	var curr_month=1;
                                 	while(curr_month<=12)
								...
								$(".date_click .year_month select").change(function() {
							                ...
							                if(set_json.today_start == "yes"&&$(this).hasClass("year")){
							                	if(dom.hasAttribute("startdom_maxdate"))
							                	{
							                		var end_time=stringTotime(dom.attr("startdom_maxdate"))-set_json.startToend_days*24*60*60*1000;
                                                    var curr_month=date_obj.getMonth()+1;
                                                    var curr_year=date_obj.getFullYear();
                                                    var item_time=new Date(curr_year,curr_month-1,1).valueOf();
                                                    var ok_month=[];
                                                    while(item_time<=end_time) 
                                                    {
                                                            if(curr_month<=12)
                                                            {
                                                            	if(curr_year==$(this).val())
                                                            	{
                                                            		ok_month.push(curr_month);
                                                            	}
                                                            	curr_month++;
                                                            	item_time=new Date(curr_year,curr_month-1,1);
                                                            }else if(curr_month>12)
                                                            {
                                                            	curr_year++;
                                                            	curr_month=1;
							                	...
							                	}else if(dom.hasAttribute("enddom_mindate"))
							                	{
							                		...
							                		var end_time=todayTime()+(set_json.limit_days>0?set_json.limit_days:0)*24*60*60*1000;                                                   var curr_month=date_obj.getMonth()+1;
                                                    var curr_month= stringTotime(dom.attr("enddom_mindate"))+set_json.startToend_days*24*60*60*1000;
                                                    var curr_year=new Date(curr_month).getFullYear();
                                                    curr_month=new Date(curr_month).getMonth()+1;
                                                    var item_time=new Date(curr_year,curr_month-1,1).valueOf();
                                                    var ok_month=[]; 
                                                    while(item_time<=end_time) 
                                                    {
							                		...
							                		if(set_json.limit_days>0)
							                		  {
							                		  			var end_month=todayTime()+set_json.limit_days*24*60*60*1000;
										                		if($(this).val()==date_obj.getFullYear())
										                		{
										                			
										                		     if(new Date($(this).val()-0+1,0,1).valueOf()<=end_month)
										                		     {
										                		     	setMonth(date_obj.getMonth()+1,12);
										                		     }else{
										                		     	setMonth(date_obj.getMonth()+1,new Date(end_month).getMonth()+1);	
										                		     }
										                			...
										                			if(new Date($(this).val()-0+1,0,1).valueOf()<=end_month)
										                			{
										                				...
										                				setMonth(1,new Date(end_month).getMonth()+1)
										                				
										                			}
										                			
										                		}
							                		  	
							                		  }else if(set_json.limit_days=="no"){
							                		  	     if($(this).val()==date_obj.getFullYear())
							                		  	     {
							                		  	     	setMonth(date_obj.getMonth()+1,12)
							                		  	     }else{
							                ...
							                }else if(set_json.today_start == "no"&&$(this).hasClass("year")){
							                	...
							                	if(dom.hasAttribute("startdom_maxdate"))
							                	{
							                		...
							                		var curr_month=1;
							                		var curr_time=new Date(curr_year,curr_month-1,1).valueOf();
							                		var ok_month=[];
							                		while(curr_month<=12)
							                		{
							                			...
							                			curr_month++;
							                			curr_time=new Date(curr_year,curr_month-1,1).valueOf();
							                			
							                		} ;
							                		console.log(ok_month);
							                		setMonth(ok_month[0],ok_month[ok_month.length-1])
							                		
							                	}else if(dom.hasAttribute("enddom_mindate")){
							                		var start_time=stringTotime(dom.attr("enddom_mindate"))+set_json.startToend_days*24*60*60*1000; 							                		var end_time=stringTotime(dom.attr("startdom_maxdate"))-set_json.startToend_days*24*60*60*1000;
							                		var curr_year=$(this).val();
							                		var curr_month=1;
							                		var next_time=new Date(curr_year,curr_month+1,1).valueOf();
							                		var ok_month=[];
							                		while(curr_month<=12)
							                		{
							                			if(curr_month<12)
							                			{
								                			...
								                			next_time=new Date(curr_year,curr_month+1,1).valueOf();
							                				
							                			}else if(curr_month==12){
							                				if(new Date(curr_year+1,0,1).valueOf()>=start_time)
							                				{
							                					ok_month.push(curr_month);
