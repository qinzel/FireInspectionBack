...
window.lCalendar = (function() {
	...
	MobileCalendar.prototype = {
		init: function(params) {
			...
			this.space_days=params.space_days;
			...
			function finishMobileDate(e) {
				...
				_self.trigger.value = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd;
		         if($(_self.trigger).attr("start-date")=="")
		         {
		         	 ...
		         	 curr_space[0]=_self.trigger.value.substring(0,4)+"-"+_self.trigger.value.substring(5,7)+"-"+_self.trigger.value.substring(8,10);
		             end_target.attr("data-lcalendar",curr_space.join(","));
		             if(end_target.val()!=""&&end_target.val().replace(/[^0-9]/ig,"")-_self.trigger.value.replace(/[^0-9]/ig,"")<0)
		             {
		             	  go_alert2("开始时间不能大于结束时间，请重新选择");
		             	  $(_self.trigger).val("")
		         	 ...
		         	 curr_space[1]=_self.trigger.value.substring(0,4)+"-"+_self.trigger.value.substring(5,7)+"-"+_self.trigger.value.substring(8,10);
		             start_target.attr("data-lcalendar",curr_space.join(","));
                     if(start_target.val()!=""&&start_target.val().replace(/[^0-9]/ig,"")-_self.trigger.value.replace(/[^0-9]/ig,"")>0)
		             {
		             	  go_alert2("开始时间不能大于结束时间，请重新选择");
		             	  $(_self.trigger).val("")
		             }
