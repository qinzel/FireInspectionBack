        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    //li标签加上title属性，展示图片尺寸
                    var title = new Array(),
                        imgName = list[i].url.substr(list[i].url.lastIndexOf('/') + 1),
                        imgSize = imgName.split(/\.|\-/);

                    if (imgSize.length == 4) {
                        title.push(imgSize[1]);
                        title.push(imgSize[2]);
                        item.setAttribute('title', "尺寸：" + title.join("×"));
                    } else {
                        item.setAttribute('title', "尺寸：原图大小");
                    }
