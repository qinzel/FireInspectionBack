...
namespace EHECD.FirePatrolInspection.Web.Areas.Admin.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            //DeviceService.Instance.SetQRCode();
            ...
            if (user == null)
            {
                return Redirect("/Admin/User/Tip");
            }
            ...
            {
                List<EHECD_Module> parentList = moduleList.Where(m => m.iPID == 0).ToList();//获取大模块
                if (parentList != null)
                {
                    foreach (EHECD_Module pModule in parentList)
                    {//遍历大模块
        ...
        /// <returns></returns>
        private string InitMenu(long iModuleID, List<EHECD_Module> modulelist)
        {
            ...
            List<EHECD_Module> childList = modulelist.Where(m => m.iPID == iModuleID).ToList();//获取子模块菜单
            if (childList != null && childList.Count > 0)
            {
                if (user.UserType == UserType.Admin)
                {//管理员 则显示所有的菜单模块
                    foreach (EHECD_Module module in childList)
                    {
                ...
                {//普通后台用户判断权限
                    ...
                    foreach (EHECD_Module module in childList)
                    {
                        ...
                        {
                            bool bIsHave = false;
                            List<EHECD_Module> ThirdList = modulelist.Where(m => m.iPID == iTempModuleID).ToList();//获取三级模块菜单
                            foreach (EHECD_Module ThirdModule in ThirdList)
                            {
