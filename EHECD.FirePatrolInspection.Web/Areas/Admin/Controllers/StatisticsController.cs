...
namespace EHECD.FirePatrolInspection.Web.Areas.Admin.Controllers
{
    public class StatisticsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.userCount = UserService.Instance.GetCount();

            #endregion

            #region 账号申请信息

            IEnumerable<EHECD_Unit> uList = UnitService.Instance.GetApplyList();
            if (uList != null && uList.Count() > 0)
            {
                ViewBag.useCount = uList.Where(o => o.iType == 0).Count();
                ViewBag.fireCount = uList.Where(o => o.iType == 1).Count();
                ViewBag.repairCount = uList.Where(o => o.iType == 2).Count();
            }
            else
            {
                ViewBag.useCount = 0;
                ViewBag.fireCount = 0;
                ViewBag.repairCount = 0;
            }

            #endregion

            ViewBag.user = UserSession.GetLogUser().sLoginName;
        ...
        public JsonResult GetRecordGridData(int iUseDeptID, string sYearNumber)
        {
            ...
            if (recList != null && recList.Count() > 0)
            {
                for (var i = 1; i <= 12; i++)
                {
                    IEnumerable<EHECD_InspectionRecord> partList = recList.Where(o => o.dCreateTime.Month == i).GroupBy(o => new { o.iDeviceID }).Select(o => o.First()).ToList();
