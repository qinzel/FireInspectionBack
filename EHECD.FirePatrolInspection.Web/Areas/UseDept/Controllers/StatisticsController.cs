...
namespace EHECD.FirePatrolInspection.Web.Areas.UseDept.Controllers
{
    public class StatisticsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            IEnumerable<EHECD_Client> uList = ClientService.Instance.GetListByType(2);
            if (uList != null && uList.Count() > 0)
            {
                ViewBag.clientCount = uList.Where(o => o.iType == 2 && o.iUnitID == user.iUnitID).Count() + uuCount;
            }
            else
            {
            ...
            ViewBag.deptCount = DeptService.Instance.GetListByUnitID(user.iUnitID).Count();

            #endregion

            #region 关联维护公司

            ViewBag.dutyList = UnitService.Instance.GetRelRepairDeptAllList(user.iUnitID);

            #endregion

            #region 单位站内信

            ViewBag.msgList = SiteMsgService.Instance.GetUnitMessageList(user.iUnitID, 1);

            #endregion

            #region 使用单位点检员申请

            ViewBag.applyCount = ClientService.Instance.GetApplyListByUnitID(user.iUnitID);
